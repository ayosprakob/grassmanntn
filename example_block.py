...
from grassmanntn import gauge2d_block as gauge
...
print("   _ini:",vol,β,m,μ,q,a,Nf,Nphi,"   ",np.real(F),"   ",np.imag(F),"   ",(T.effective_shape[0],T.effective_shape[1]))
for i in range(cgsteps):
    ...
    print(process+":",vol,β,m,μ,q,a,Nf,Nphi,"   ",np.real(F),"   ",np.imag(F),"   ",(T.effective_shape[0],T.effective_shape[1]),"   ",'{:.3g}'.format(err),"   ",gtn.time_display(time.time()-t0))
